...
class MergedTransactions:
    ...
    def merge_transactions(file_path: str) -> None:
        ...
        try:
            ...
            os.makedirs(config.NEW_TRANSACTIONS_DIR, exist_ok=True)
            transaction_files = sorted([f for f in os.listdir(config.NEW_TRANSACTIONS_DIR) if f.endswith(".txt")])
            ...
            with open(file_path, "w") as merged_file:
                for file in transaction_files:
                    ...
                    with open(f"{config.NEW_TRANSACTIONS_DIR}{file}", "r") as f:
                        for line in f:
                    ...
                    if saw_EOF:
                        os.remove(f"{config.NEW_TRANSACTIONS_DIR}{file}")
